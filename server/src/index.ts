...
export async function maybeTriggerAssistants(
  ...
  // ===== P1 新增分支:频道有 active task → 优先派给该 task 的 assignee =====
  // 用户痛点:派 Aria 做 PPT 后,回"不够精美继续优化"没人理 — 因为 build intent 不命中 H2。
  // 修复:项目频道里有 status in (todo/doing/review) 的最近 task,task.assigneeId 是助理且在频道 + 真人 + 无 @ + 顶层
  //   → 让该 assignee 走 generateReply 自然回复(把"再做一版"等延续语义接住)
  // 这是 v4 "AI 团队" 理念落地:**派工 = 绑定**,后续无 @ 默认归 task owner,别人靠 @ 喊。
  if (
    ...
    if (activeTask && activeTask.assigneeId) {
      const owner = assistantById.get(activeTask.assigneeId)
      if (
        owner &&
        owner.autoRespond &&
        canGenerate({
  ...
  // 让下面 H2 知道:已经在 P1 里安排了 owner 接手 → 跳过 H2 自动派
  const p1Routed = targetIds.length > 0 && !mentions.directed
  if (p1Routed) {
    // 把 mentions.directed 视为 true(下方 routedIds 主动响应不再启动)
    ;(mentions as any).directed = true
  }
...
app.post('/api/templates/generate-pptx-ai', async (req, reply) => {
  ...
  // P1:同时真创建一个 Task(让助理成为该频道的 active task owner,后续无 @ 消息自动响应)
  const jobId = randomUUID()
  let dispatchMsgId: string | null = null
  let ackMsgId: string | null = null
  let pptTaskId: string | null = null
  if (channelId) {
    try {
      // P1.a:把助理加入频道成员(若不在)— 否则 maybeTriggerAssistants 找不到她
      const exist = await prisma.channelMember.findFirst({
        where: { channelId, userId: assistant.id },
      })
      if (!exist) {
        await prisma.channelMember.create({
          data: { channelId, userId: assistant.id },
        })
      }

      // P1.b:真创建 Task — title=topic,assigneeId=助理,status=todo
      // 这样频道有 active task,后续 maybeTriggerAssistants 能优先派该 assignee 响应
      const pptTask = await prisma.task.create({
        data: {
      ...
      pptTaskId = pptTask.id
      await writeAudit({
        type: 'ppt.task_created',
        summary: `${me.name} 指派 ${assistant.name} 做 PPT「${topic.slice(0, 60)}」`,
        actorId: me.id,
        taskId: pptTask.id,
        payload: { channelId, assistantId: assistant.id, topic, jobId },
      })
  ...
  void (async () => {
    try {
      ...
      // 任务标失败(保留 active 状态,让用户能看到失败原因后再决定重试 / 取消)
      if (pptTaskId) {
        await prisma.task.update({ where: { id: pptTaskId }, data: { status: 'todo' } }).catch(() => {})
      }
    }
  })()
